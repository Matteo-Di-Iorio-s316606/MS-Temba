...
OUTDIR="/srv/storage/stars@storage3.sophia./mdiiorio/masters-thesis/Traineeship/MS-Temba/runs/multithumos/clip/seed0"
mkdir -p "$OUTDIR"
...
python MSTemba_main.py \
  ...
  -resume "$RESUME" \
  -save_every 1 \
  -rgb_root "/srv/storage/stars@storage3.sophia./mdiiorio/masters-thesis/Traineeship/MS-Temba/data/hf_features/Temporal_Action_Detection/multithumos_features/multithumos_features_clip" \
  -num_clips 256 \
  -skip 0 \
  -comp_info False \
  -epochs 50 \
